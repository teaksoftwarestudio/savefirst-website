...
/* ── Transactions screen (inline, same as page.tsx row 4) ── */
...
/* each slide occupies 1 full viewport height of scroll travel */
const SCROLL_PER_SLIDE = 1;
...
export default function HorizontalShowcase() {
  ...
  /* scroll tracking anchored to this section */
  ...
  /* drive the horizontal translate of the track */
  const xPct = useTransform(smoothProgress, [0, 1], ["0%", `-${(TOTAL - 1) * 100}%`]);

  /* derive active slide index for copy + dots */
  useEffect(() => {
  ...
  /* fractional progress within current slide [0,1] for progress bar */
  const segProgress = useTransform(smoothProgress, (v) => {
    return (v * (TOTAL - 1)) % 1;
  });

  return (
    /* outer: tall enough for TOTAL viewports of scroll */
    <div
      ref={outerRef}
      className="hsc-outer"
    ...
    >
      ...
      <div className="hsc-sticky">
        ...
        {/* ambient background glow that shifts with slide */}
        <motion.div
        ...
        <div className="hsc-layout">
          ...
          <div className="hsc-copy-col">
            ...
            {/* dot nav */}
            <div className="hsc-dots" role="tablist" aria-label="Feature slides">
              {SLIDES.map((s, i) => (
                <div
                  key={i}
                  role="tab"
                  aria-selected={i === active}
                  className={`hsc-dot${i === active ? " on" : ""}`}
                  aria-label={s.tag}
                />
              ))}
            </div>
